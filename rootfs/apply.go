...
func ApplyLayer(snapshots Snapshotter, mounter Mounter, rd io.Reader, parent digest.Digest) (digest.Digest, error) {
	...
	chainID := diffID
	if parent != "" {
		chainID = ChainID([]digest.Digest{parent, chainID})
	}
...
func Prepare(snapshots Snaphotter, mounter Mounter, layers []distribution.Descriptor,
	// TODO(stevvooe): The following functions are candidate for internal
	// object functions. We can use these to forumlate the beginnings of a
	// rootfs Controller.
	//
